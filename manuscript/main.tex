...
This paper studies that existence question for a specified weighted Euler discretization. The issue is separate from the established mismatch between composed intermediate distributions and forward-noised target distributions, and from learned score error \citep{linhart2026,soiffer2026}. Oracle experiments first isolate the random-weight effect. A controlled conditional-density learning study then uses exact noised scores of learned mixtures, allowing posterior learning error and particle error to be measured separately. We also distinguish finite normalizing constants from small endpoint error: a valid discretization may still be inaccurate, and a finite-particle sample may appear accurate when its population operator is undefined.

Our main result makes this distinction computable. For scalar Gaussian factors with nonnegative residual precisions, the largest variance reachable over all finite batch histories evolves by a two-choice recursion. The choices are batches containing only the weakest or only the strongest factor. These events have positive probability for every finite with-replacement batch size, so increasing the batch size can reduce estimator mean squared error without changing the integrability certificate. A closed-form four-factor example exhibits this separation in a single step. Uniform sampling without replacement instead admits a batch-size-dependent prefix/suffix recursion. For non-Gaussian factors with a shared variance within each Gaussian mixture, an affine control preserves the deterministic quadratic tail and the full-factor strict integrability domain. The empirical study reports both the certificates and the observed particle errors.
...
Random-weight particle filters have long treated positive stochastic weights explicitly \citep{fearnhead2010}. Convergence with unbounded importance weights requires moment conditions \citep{mbalawata2016}, and particle stability analyses impose assumptions on the underlying Feynman--Kac flow \citep{whiteley2013}. Subsampling SMC uses approximately unbiased annealed likelihood estimators and auxiliary-variable updates \citep{gunawan2020}. These works establish the relevance of positivity, moments, and exponentiation bias. Our contributions are exact extreme-batch and prefix/suffix certificates for a particular compositional weighted Euler operator, their differing batch-size dependence, and preservation of the full-factor strict domain through deterministic tail curvature. The Gaussian integral and unbiased quadratic estimator are classical calculations.

\section{Discussion}
The certificate distinguishes three questions that are often evaluated together: existence of a population target, accuracy of its time discretization, and behavior of a finite particle approximation. With-replacement subsampling can fail the first question while producing finite samples and decreasing local mean squared error. Without replacement, batch size changes the certificate, and all subset sizes in our four-factor one-step example are finite. Tail control preserves the full-factor strict domain for the stated mixture class, but accuracy and wall-clock cost remain empirical questions. The learned experiment requires a density architecture with known Gaussian tails; it is not a certificate for an arbitrary neural score. The analysis is restricted to scalar or diagonal product tails and a fixed weighted Euler construction. Noncommuting covariances, adaptive batches and learned models beyond this tail class require additional arguments.
...
\appendix
\input{appendix}
\input{baseline_tables}
\input{extension_protocol}
\input{training_results}
\input{extension_results}
\end{document}
